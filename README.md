...
![Gitea Comment](/pull-request-comment.png)
...
Builds the tool and runs it against the seeded pull request, reporting the violation in
`scripts/example-checkstyle.xml` (which matches the file/line `gitea-setup.sh` seeds, so it works
out of the box); extra arguments are passed straight through. Check the result at
`http://localhost:3000/<owner>/<repo>/pulls/<pr>`.
