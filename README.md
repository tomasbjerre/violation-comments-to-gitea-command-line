# Violation Comments To Gitea Command Line

[![NPM](https://img.shields.io/npm/v/violation-comments-to-gitea-command-line.svg?style=flat-square)](https://www.npmjs.com/package/violation-comments-to-gitea-command-line)
[![Maven Central](https://img.shields.io/maven-central/v/se.bjurr.violations/violation-comments-to-gitea-command-line.svg?label=Maven%20Central)](https://central.sonatype.com/artifact/se.bjurr.violations/violation-comments-to-gitea-command-line)
[![NPM Downloads](https://img.shields.io/npm/dm/violation-comments-to-gitea-command-line.svg?style=flat)](https://www.npmjs.com/package/violation-comments-to-gitea-command-line)
[![Docker Pulls](https://badgen.net/docker/pulls/tomasbjerre/violation-comments-to-gitea-command-line?icon=docker&label=pulls)](https://hub.docker.com/r/tomasbjerre/violation-comments-to-gitea-command-line/)

Report static code analysis to Gitea. It uses the [Violations Lib](https://github.com/tomasbjerre/violations-lib).

Unlike the other `violation-comments-to-*` tools, this one ships as a single Gradle module: the
CLI (`se.bjurr.violations.main`) and the Gitea integration (`se.bjurr.violations.comments.gitea.lib`)
are separate Java packages within it, rather than separate published artifacts. There's no
existing Java Gitea client mature enough to depend on, so the client in
`se.bjurr.violations.comments.gitea.lib.client` is hand-written, using the JDK's built-in
`java.net.http.HttpClient` and Jackson - no HTTP client dependency needed.

| Version          | Java Version |
| -----------------| ------------ |
| 0.1.0 <= version | 17           |

- The runnable `jar` can be found in [Maven Central](http://search.maven.org/#search%7Cga%7C1%7Ca%3A%22violation-comments-to-gitea-command-line%22) and used like `java -jar violation-comments-to-gitea-command-line-*.jar ....`.
- The runnable can be found in [NPM](https://www.npmjs.com/package/violation-comments-to-gitea-command-line).
- The `Docker` image can be found in [Dockerhub](https://hub.docker.com/r/tomasbjerre/violation-comments-to-gitea-command-line) and used like `docker run --mount src="$(pwd)",target=/usr/src/violation-comments-to-gitea-command-line,type=bind tomasbjerre/violation-comments-to-gitea-command-line:X`.

Run it with:

```shell
npx violation-comments-to-gitea-command-line \
 -pat TOKENHERE \
...
```shell
Usage: violation-comments-to-gitea-command-line [-h] [-show-debug-info]
       ...
       -o=<owner> [-password=<password>] [-pat=<personalAccessToken>]
       ...
       -url=<giteaUrl> [-username=<username>] [-i=<ignorePaths>]...
  ...
  -s, -severity=<minSeverity>
      ...
      -url, -server-url=<giteaUrl>
                             Base URL of the Gitea instance, e.g. https://gitea.
                               example.com
      -username=<username>
...
A local Gitea instance (Docker) is used both to manually try the tool against a real server and
to capture real request/response pairs for the test suite. Everything needed is in `scripts/`;
the commands below are all you need.

```shell
./scripts/gitea-start.sh   # start Gitea on http://localhost:3000 (keeps data across restarts)
./scripts/gitea-setup.sh   # seed an admin user, API token, test repo and a demo pull request
```

`gitea-setup.sh` writes the token, owner, repo and pull request number it created to
`scripts/.env` (gitignored - it holds a live API token) for reuse by other scripts and by manual
`curl`/CLI runs. It's safe to re-run either script.

```shell
./scripts/gitea-run-cli.sh -v CHECKSTYLE ./some/folder ".*checkstyle\.xml$" Checkstyle
```

Builds the tool and runs it against the seeded pull request; extra arguments are passed straight
through. Check the result at `http://localhost:3000/<owner>/<repo>/pulls/<pr>`.

```shell
./scripts/gitea-stop.sh    # stop the container, keep its data
./scripts/gitea-reset.sh   # stop it and wipe all data, for a clean slate
```

### Tests

`./gradlew test` runs four test classes, in two groups:

- `GiteaClientTest` and `GiteaCommentsProviderWireMockTest` need neither Docker nor a running
  Gitea. The former unit-tests the pure helpers (diff splitting, JSON escaping); the latter
  drives the whole provider over real HTTP against WireMock, stubbed with real request/response
  fixtures captured from a local Gitea instance (`src/test/resources/gitea`). These run in CI.

- `GiteaIntegrationTest` and `GiteaCliIntegrationTest` target a real, live Gitea instance -
  the former through the internal Java API, the latter through the actual command-line entry
  point (`Runner`/`Main`), parsing a real Checkstyle report file. Both read connection details
  from `scripts/.env` (see [`LocalGiteaEnv`](src/test/java/se/bjurr/violations/comments/gitea/lib/LocalGiteaEnv.java))
  and **skip themselves** (a JUnit assumption failure, not a test failure) when that file isn't
  there - so they're always enabled, no source edits needed, and stay safe to run in CI or
  anywhere else Gitea isn't running. To actually run them: `./scripts/gitea-start.sh &&
  ./scripts/gitea-setup.sh`, then `./gradlew test` as usual. They're also what to use as the
  starting point for re-capturing the WireMock fixtures if Gitea's API shape ever changes.
