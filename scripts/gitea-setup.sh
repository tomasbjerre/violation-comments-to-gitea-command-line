...
ensure_admin_user() {
  if docker compose exec -T --user git gitea gitea admin user list 2>/dev/null | awk '{print $2}' | grep -qx "$GITEA_ADMIN_USER"; then
...
JAVA
    git commit --quiet -am "feat: tweak MyClass for violation comments demo"
